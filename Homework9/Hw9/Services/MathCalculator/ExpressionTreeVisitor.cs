...
public class ExpressionTreeVisitor : ExpressionVisitor
{
    ...
    {
        var leftExpr = Task.Run(async () => { Thread.Sleep(4000); return Expression.Lambda<Func<double>>(left).Compile().Invoke(); });
        var rightExpr = Task.Run(async () => { Thread.Sleep(4000); return Expression.Lambda<Func<double>>(right).Compile().Invoke(); });
